  ...
  case "$1" in
    ...
    *)
      ...
      if [[ "$use_tmux" -eq 1 ]]; then
        ...
        if [[ -n "$TMUX" ]]; then
          tmux new-window -c "$dir" "${tmux_args[@]}" # 已在 tmux 中时打开新窗口
        else
          tmux new-session -c "$dir" "${tmux_args[@]}" # 否则创建并进入新会话
        fi
        return
      fi
      cd "$dir" || return
...
注意：`--print/-p`、`--claude/-cc`、`--codex/-cdx`、`--tmux/-t` 的分支逻辑在 wrapper 层处理；二进制侧通过 `--print` 统一解析并输出路径。纯打印只输出路径并保持当前目录不变；Claude 与 Codex 冲突时 wrapper 返回状态 2。使用 tmux 参数时，wrapper 在现有 tmux 客户端内打开新窗口，否则创建并进入新会话；若同时指定 Claude 或 Codex，则将对应编辑器作为 tmux 窗口的启动命令。wrapper 需导出标记变量（如 `export ZHAO_SHELL_WRAPPED=1`），供 `doctor` 和 onboarding 检测 wrapper 是否生效。
...
| 命令                  | 规格                                                                                                                                                                                                                                                                                                                                                       |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `zhao <query>`        | 核心检索。resolveProject → 输出路径（wrapper 完成 cd）。flags：`--print/-p`（仅打印路径）、`--claude/-cc`（cd 后启动 Claude Code）、`--codex/-cdx`（cd 后启动 Codex）、`--tmux/-t`（在 tmux 内打开新窗口，外部调用时创建并进入新会话）。Claude 与 Codex 不能同时使用；tmux 与编辑器参数同时出现时，在 tmux 窗口中启动对应编辑器。无 query 时弹全量选择列表 |
...
| 命令                           | 规格                                                                                                                                                                                                                                                                                 |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `zhao ci [test\|prod] [query]` | 打开构建平台。参数消歧：首个位置参数 ∈ {test, prod} 则为环境，否则视为 query；环境默认 test。URL 解析：projects.yaml links（`ci-test`/`ci-prod`）优先 → ciTemplates 模板填充 `{group}`/`{name}` 兜底 → 都没有则报错并提示配置方法。支持 `--copy/-c`、`--print/-p`，降级行为同 browse |
| `zhao tag <project>`           | 手动元数据录入："检索失败即录入"的入口。flags：`--domain`、`--kw`、`--alias`（均可多值）、`--rm-domain`（写入 blockedDomains）、`--ci-test` / `--ci-prod`（写入 links）。project 参数支持模糊匹配 + 多命中选择                                                                       |
