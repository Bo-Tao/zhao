...
import { createSpinner, promptConfirm, promptText } from '../ui/prompts.js'
...
export const ensureOnboarded = async (
  ...
  if (
    ...
    if (shouldSetup) {
      ...
      process.stderr.write(
        '请重新打开终端或 source 对应 rc 文件使 wrapper 生效。\n',
      )
    }
  }
  ...
  const shouldScan = await promptConfirm('索引不存在，现在重建吗？', true)
  if (!shouldScan) {
