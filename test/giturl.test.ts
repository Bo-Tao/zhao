...
describe('git remote 解析', () => {
  ...
  it('解析 HTTPS remote 并移除 .git', () => {
    expect(
      remoteToProjectId('https://git.100tal.com/bigclass_xuefu_fe/tal-npm.git'),
    ).toBe('git.100tal.com/bigclass_xuefu_fe/tal-npm')
  })

  it('把 SSH 与 HTTPS remote 统一转换为网页 URL', () => {
    expect(remoteToWebUrl('git@git.100tal.com:group/repo.git')).toBe(
      'https://git.100tal.com/group/repo',
    )
    expect(remoteToWebUrl('https://git.100tal.com/group/repo.git')).toBe(
      'https://git.100tal.com/group/repo',
    )
  })
